...
class GPTLosses(BaseLosses):
    ...
    def __init__(self, cfg, stage, num_joints, **kwargs):
        ...
        for loss in losses:
            ...
            elif loss.split("_")[1] in ["commit", "loss"]:  # , 'gpt', 'm2t2m', 't2m2t'
                losses_func[loss] = CommitLoss
            # we use the huggingface loss directly.
            # elif loss.split('_')[1] in ['loss']:
    ...
    def update(self, rs_set):
        ...
        if self.stage in ["vae"]:
            ...
            total += self._update_loss(
                "vq_commit", rs_set["loss_commit"], rs_set["loss_commit"]
            )

        if self.stage in ["lm_pretrain", "lm_instruct"]:
            total += self._update_loss(
                "gpt_loss", rs_set["outputs"].loss, rs_set["outputs"].loss
            )
