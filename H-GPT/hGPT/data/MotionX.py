...
def motionx_collate(batch):
    ...
    # Name
    adapted_batch = {"name": [b[0] for b in notnone_batches]}
        ...
        adapted_batch.update(
            {
                "motion": collate_tensors(
                    [torch.tensor(b[1]) for b in notnone_batches]
                ),
        ...
        adapted_batch.update(
            {
                "m_tokens": collate_tensors(
                    [torch.tensor(b[3]) for b in notnone_batches]
                ),
        ...
        adapted_batch.update(
            {
                "text_len": collate_tensors(
                    [torch.tensor(b[6]) for b in notnone_batches]
                ),
                "tokens": [b[7] for b in notnone_batches],
                "word_embs": collate_tensors(
                    [torch.tensor(b[8]) for b in notnone_batches]
                ).float(),
                "pos_ohot": collate_tensors(
                    [torch.tensor(b[9]) for b in notnone_batches]
                ).float(),
            }
        )
...
class MotionXDataModule(BASEDataModule):
    def __init__(self, cfg, **kwargs):
        ...
        if cfg.DATASET.MOTIONX.EVAL_MEAN_STD_PATH == "":
            ...
            eval_mean_std_root = (
                cfg.DATASET.MOTIONX.EVAL_MEAN_STD_PATH
            )  # only use for motionx test
        ...
        self.hparams.stage = (
            cfg.TRAIN.STAGE
        )  # stage "vae" , "lm_pretrain", "lm_instruct"
