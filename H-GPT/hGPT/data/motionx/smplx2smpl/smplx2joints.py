...
def process_smplx_322_data(
    ...
    zero_expr = (
        torch.zeros((batch_size, num_frames, 10)).float().to(device)
    ...
    if norm_global_orient:
        for i in range(num_frames):
            (
                body_parms["root_orient"][:, i : i + 1, :],
                body_parms["trans"][i : i + 1],
    ...
    if use_flame:
        output = smplx_layer(
            ...
            flame_betas=body_parms["face_shape"],
            flame_expression=body_parms["face_expr"],
        )
        #
...
if __name__ == "__main__":
    ...
    mesh = trimesh.Trimesh(
        vertices=vert.squeeze().detach().cpu().numpy()[50], faces=faces
    )
