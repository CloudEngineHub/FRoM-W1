...
class Text2MotionDatasetToken(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        print(f">>> Split {split} Loaded {len(self.name_list)} Samples.")
...
class Text2MotionDatasetTokenDist(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        def process_single_item(item):
            ...
            try:
                ...
                if (len(motion)) < self.min_motion_length or (
                    ...
                    if SHOW_INFO:
                        print(
                            f"Length out of range: {name}. curr: {len(motion)} min: {self.min_motion_length} max: {self.max_motion_length}. skipped."
                        )
                    return

                # 创建数据项
                data_item = {"motion": motion, "length": len(motion), "name": name}
        ...
        if not debug:
            pbar = tqdm(
                total=len(items_to_process), desc=f"Loading Dataset {split_file}"
            )
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            # 提交所有任务
            futures = [
                executor.submit(process_single_item, item) for item in items_to_process
            ]
        ...
        print(
            f"Successfully loaded {len(self.name_list)} items out of {len(self.id_list)}"
        )
