    ...
    def __init__(
        ...
        **kwargs,
        ...
        for idx, name in enumerator:
            ...
            try:
                motion = np.load(
                    pjoin(motion_feat_path, name + ".npy"), allow_pickle=True
                )
            ...
            if (len(motion)) < self.min_motion_length or (
                len(motion) >= self.max_motion_length
            ):
                if SHOW_INFO:
                    print(
                        f"Length out of range: {name}. curr: {len(motion)} min: {self.min_motion_length} max: {self.max_motion_length}. skiped."
                    )
                continue
            ...
            with cs.open(pjoin(text_path, name + ".txt"), "r") as f:
                ...
                for line_idx, line in enumerate(lines):
                    ...
                    if f_tag == 0.0 and to_tag == 0.0:
                        ...
                        motion_new = motion[
                            int(f_tag * self.fps) : int(to_tag * self.fps)
                        ]
        ...
        **kwargs,
        ...
        def process_single_item(name_idx_name):
            ...
            try:
                # load motion
                motion = np.load(
                    pjoin(motion_feat_path, name + ".npy")
                )  # , allow_pickle=True
                # try:
                    ...
                    with cs.open(pjoin(text_path, name + ".txt"), "r") as f:
                        ...
                        for line_idx, line in enumerate(lines):
                            ...
                            if f_tag == 0.0 and to_tag == 0.0:
                                ...
                                data_queue.put(
                                    ("segmented", new_name, item_data, len(motion_new))
                                )
...
def unit_test():
    ...
    #     mean = 0.0,
    #     std = 1.0,
    #     split_file = '/inspire/hdd/project/embodied-multimodality/public/pli/FRoM-W1/datasets/humanml3d-x/data/test.txt',
    #     w_vectorizer = None,
    #     max_motion_length = 300,
    #     min_motion_length = 60,
    #     max_text_len = 20,
    #     unit_length = 4,
    #     motion_dir = '/inspire/hdd/project/embodied-multimodality/public/pli/FRoM-W1/datasets/humanml3d-x/data/new_joint_vecs',
    #     text_dir = '/inspire/hdd/project/embodied-multimodality/public/pli/FRoM-W1/datasets/humanml3d-x/data/texts'
    # )
