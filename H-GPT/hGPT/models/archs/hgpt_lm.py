...
class MLM(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if new_token_type == "insert":
            ...
            shared = NewTokenEmb(self.language_model.shared, self.m_codebook_size + 3)
    ...
    def forward(
        ...
        if self.lm_type == "encdec":
            return self.forward_encdec(
                texts, motion_tokens, motion_tokens_len, tasks, cot
            )
    ...
    def forward_dec(
        ...
        motion_strings = self.motion_token_to_string(
            motion_tokens, motion_tokens_len, self.pkeep
        )
        ...
        if condition == "text":
            ...
            inputs, outputs = self.template_fulfill(
                tasks, motion_tokens_len, motion_strings, texts, cot
            )
            inputs = [item + "\n" for item in inputs]
            labels = []
            for i in range(len(inputs)):
                labels.append(
                    inputs[i] + outputs[i] + self.tokenizer.eos_token
                )  # + '\n'
        # print (labels)
        # print ("* " * 20)
        # # exit(0)
        ...
        if self.mask_inputs:
            ...
            if instruction_attn_mask.shape[1] < labels.shape[1]:
                padding = torch.zeros(
                    batch_size,
                    labels.shape[1] - instruction_attn_mask.shape[1],
                    dtype=instruction_attn_mask.dtype,
                    device=instruction_attn_mask.device,
                )
        ...
        if self.cont_loss:
            ...
            last_hidden_states = outputs["hidden_states"][-1][
                torch.arange(batch_size), last_one_indices
            ]
    ...
    def generate_conditional(
        ...
        if task in ["t2m", "m2m", "pred", "inbetween"]:
            ...
            inputs, outputs = self.template_fulfill(
                tasks, lengths, motion_strings, texts, cot
            )
            # print ("* " * 10)
            # print ("inputs: ")
            # print (inputs)
            ...
            )  # TODO: check here, do sample = False
    ...
    def motion_token_to_string(
        ...
        for i in range(len(motion_token)):
            motion_i = (
                motion_token[i].cpu()
                if motion_token[i].device.type == "cuda"
                else motion_token[i]
            )
    ...
    def motion_string_to_token(self, motion_string: List[str]):
        ...
        for i in range(len(motion_string)):
            ...
            output_string.append(
                motion_string[i].replace(string, "<Motion_Placeholder>")
            )
    ...
    def template_fulfill(self, tasks, motion_tokens_len, motion_strings, texts, cot):
        inputs = []
        outputs = []
        for i in range(len(motion_tokens_len)):
            ...
            inputs.append(
                self.placeholder_fulfill(
                    input_template, length, motion_strings[i], texts[i], cot[i]
                )
            )
            outputs.append(
                self.placeholder_fulfill(
                    output_template, length, motion_strings[i], texts[i], cot[i]
                )
            )
    ...
    def placeholder_fulfill(
        ...
        motion_predict_head = (
            ">".join(motion_splited[:predict_head])
            + f"><motion_id_{self.m_codebook_size+1}>"
        )
        ...
        motion_masked = (
            ...
            + ">"
            + f"<motion_id_{self.m_codebook_size+2}>" * (masked_tail - masked_head)
