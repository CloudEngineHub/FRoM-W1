...
class NewTokenEmb(nn.Module):
    ...
    def __init__(
        ...
        with torch.no_grad():
            self.text_embeddings.weight.data[: old_embeddings.num_embeddings] = (
        ...
        with torch.no_grad():
            self.motion_embeddings.weight.data[: self.old_num_tokens] = torch.zeros(
                new_num_tokens,
                self.embedding_dim,
                dtype=old_embeddings.weight.dtype,
                device=old_embeddings.weight.device,
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        with torch.no_grad():
            self.motion_embeddings.weight.data[: self.old_num_tokens] = torch.zeros(
                self.new_num_tokens,
                self.embedding_dim,
                dtype=self.motion_embeddings.weight.dtype,
                device=self.motion_embeddings.weight.device,
            )

        self.motion_embeddings.weight.data[self.old_num_tokens :] = (
            self.word2motionProj(
                self.text_embeddings.weight.data[: self.old_num_tokens].permute(1, 0)
            ).permute(1, 0)
        )
